...
class NLPQLP(Optimizer):
	...
	def __solve__(self, opt_problem={}, sens_type='FD', store_sol=True, store_hst=False, hot_start=False, disp_opts=False, sens_mode='', sens_step={}, *args, **kwargs):
		...
		**Keyword arguments:**
		...
		gg = numpy.zeros([int(mmax),int(ll)])
		df = numpy.zeros([int(nmax)])
		dg = numpy.zeros([int(mmax), int(nmax)])
		uu = numpy.zeros([int(mnn2)])
		cc = numpy.zeros([int(nmax),int(nmax)])
		dd = numpy.zeros([int(nmax)])
		...
		wa = numpy.zeros([int(lwa)], numpy.float)
		lkwa = numpy.array([25+nmax], numpy.int)
		kwa = numpy.zeros([int(lkwa)], numpy.intc)
		lactiv = numpy.array([2*mmax+10], numpy.int)
		active = numpy.zeros([int(lactiv)], numpy.bool)
